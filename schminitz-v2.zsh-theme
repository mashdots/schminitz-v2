...
#
# A cleaned up, more functional schminitz ZSH theme, adapter from the Agnoster
# theme, which uses powerline fonts.
#
# # README
#
# In order for this theme to render correctly, you will need a
# [Powerline-patched font](https://github.com/Lokaltog/powerline-fonts).
# [Fira Code v0.6+](https://github.com/tonsky/FiraCode) will work as well. Make
# sure you have a recent version: the code points that Powerline uses changed in
# 2012, and older versions will display incorrectly, in confusing ways.
#
# # Goals
#
...
prompt_segment() {
  # ↓----- OLD CODE -----↓ <-- enable this to change the prompt background
  # local bg fg
  # [[ -n $1 ]] && bg="%K{$1}" || bg="%k"
  # [[ -n $2 ]] && fg="%F{$2}" || fg="%f"
...
### Prompt components
# Each component will draw itself only if information needs to be shown

# Context: user@hostname (who am I and where am I)
# prompt_context() { # <-- enable this to show username info
...
prompt_git() {
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    echo -n "${ref/refs\/heads\//$PL_BRANCH_CHAR }${vcs_info_msg_0_%% }$(git_prompt_ahead)${mode}"
  fi
}
